...
Adder_Subtractor::Adder_Subtractor(uint16_t num_bits, const std::string& name) : Device(num_bits, name)
{
    ...
    // Create NOR gate for zero flag (give it a hierarchical name)
    {
        std::ostringstream zfn_name;
        if (!name.empty())
            zfn_name << "zero_flag_nor_in_" << name;
        else
            zfn_name << "zero_flag_nor_in_adder_subtractor";
        zero_flag_nor = new NOR_Gate(num_bits, zfn_name.str());
    }
    
    // Array of single-bit Full_Adder_Subtractors
    adder_subtractors = new Full_Adder_Subtractor*[num_bits];
    std::ostringstream fas_name;
    for (uint16_t i = 0; i < num_bits; ++i)
    {
        fas_name.str("");
        fas_name.clear();
        fas_name << "full_adder_subtractor_" << i << "_in_adder_subtractor";
        adder_subtractors[i] = new Full_Adder_Subtractor(fas_name.str());
    }
    ...
    for (uint16_t i = 0; i < num_bits; ++i)
    {
        and_name.str("");
        and_name.clear();
        and_name << "output_and_" << i << "_in_adder_subtractor";
        output_AND_gates[i] = new AND_Gate(2, and_name.str());
    }
